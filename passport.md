...
Passport 引入一個[認證 Guard](/docs/{{version}}/authentication#adding-custom-guards)，會去驗證傳入的請求上的 Access Token。你一旦使用 `passport` 驅動來設定 `api` Guard，你只需要在任意路由上指定 `auth:api` 中介層，該中介層會需要一組有效的 Access Token：
