...
使用 `first` 方法，可以建立給定視圖陣列中存在的的第一個視圖。這有利於覆寫或客製化你的應用程式或套件的視圖。

    return view()->first(['custom.admin', 'admin'], $data);

你也可以透過 `View` [facade](/docs/{{version}}/facades) 來呼叫這個方法：
...
用上面的方式傳遞資料時，資料必須是一個鍵值對的陣列。在你的視圖中，你可以用相對應的鍵名取用值，像是 `<?php echo $key; ?>`。你也可以用另一個替代的語法來傳遞資料陣列，在 `view` 輔助函式使用 `with` 來傳遞額外資料給視圖：
...
舉例來說，讓我們在[服務提供者](/docs/{{version}}/providers)內註冊視圖組件。我們將使用 `View` facade 來取得底層 `Illuminate\Contracts\View\Factory` contract 實作。請注意，Laravel 沒有預設的目錄來放置視圖組件。你可以自由的把它們放在你想要的地方。舉例來說，你可以建立一個 `app/Http/View/Composers` 目錄：
...
預設情況下，視圖是需要被編譯的。在執行渲染視圖的請求時，Laravel 會去確認是否有已編譯的視圖版本。如果有找到檔案，Laravel 會去確認未編譯的視圖是否有比已編譯過的視圖還要新。如果找不到已編譯視圖或未編譯視圖有被修改過，Laravel 會重新編譯視圖。

在請求階段編譯視圖會降低效能，所以 Laravel 提供 Artisan `view:cache` 指令來事先編譯應用程式所有用到的視圖。為了優化效能，你可以在部署階段執行下列指令：

    php artisan view:cache

可以使用 `view:clear` 指令來清除視圖快取：
