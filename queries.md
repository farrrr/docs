...
`get` 方法回傳 `Illuminate\Support\Collection` 實例，這包含每個都是 PHP `StdClass` 物件實例的結果。你可以依欲存取欄位的物件屬性來存取每個欄位的值：
...
如果你正需要從資料庫中取得一筆完整記錄，你可以使用 `first` 方法。這個方法會回傳一個 `StdClass` 物件：
...
如果你想要存取包含單一欄位的集合，你可以使用 `pluck` 方法。在本範例中，我們會存取 roles 的標題集合：
...
如果你需要處理上千筆資料庫記錄，請考慮使用 `chunk` 方法。這個方法會在每次取得被分塊的結果的時候，將每個分塊送至`閉包`中處理。這個方法對於撰寫處理上千筆記錄的 [Artisan 指令](/docs/{{version}}/artisan)來說是相當有用的。例如，讓我們每次以一百記錄為分塊單位的方式來處理整個 `user` 資料表：
...
查詢建構器也提供各種聚合方法，像是 `count`、`max`、`min`、`avg` 和 `sum`。你可以在建構你的查詢後呼叫這些的任何方法：
...
當然，你可能不總是想要從資料庫中選擇所有欄位。使用 `select` 方法，能讓你為查詢指定一個自訂 `select` 子句：
...
不僅能使用 `DB::raw`，你也可以使用下列方法來插入原生表達式查詢的各個部分。
...
該查詢建構器也可以被用於撰寫 join 陳述句。要處理基本的「inner join」，你可以在查詢建構器實例上使用 `join` 方法。傳入 `join` 方法的第一個參數是你需要連接的資料表名稱。而其餘的參數則是為連接所指定欄位的搜尋條件。當然，如你所見，你能在一個查詢中連接多個資料表：
...
如果你想要要處理「left join」而不是「inner join」，請使用 `leftJoin` 方法。`leftJoin` 方法與 `join` 方法有相同的使用方式：
...
要處理「cross join」，請使用 `crossJoin` 方法，並填入你想要交叉連接的資料表名稱。交叉連接會在第一個資料表與要連接的資料表之間產生一個笛卡爾積：
...
如果你也想要指定更高階的 join 子句，你可以開始將`閉包`作為傳入 `join` 方法的第二個參數。`閉包`會接收 `JoinClause` 物件，這可以讓你指定在 `join` 子句上的搜尋條件：
...
如果你想要在你的連接使用「where」風格的子句，你可以在一個 join 上使用 `where` 和 `orWhere` 方法。這些方法不是比較兩個欄位，而是將欄位與一個值進行比較：
...
你可以在查詢中新增 `or` 子句來將 where 的搜尋條件連結再一起。`orWhere` 方法和 `where` 方法接受相同的參數：
...
`whereColumn` 方法也能傳入多個條件的陣列。這些條件會被連接使用 `and` 運算子：
...
如你所見，將`閉包`傳入 `orWhere` 方法，告訴查詢產生器開始一個約束分組。此`閉包`接收查詢產生器的實例，你可以用它來設定應包含在括號分組中的約束。上面的例子會產生以下的 SQL：
...
`whereExists` 方法可以讓你撰寫 `where exists` SQL 子句。`whereExists` 方法接受一個`閉包`參數，它會接收查詢產生器實例，讓你可以定義應放在「exists」SQL 子句中的查詢：
...
要限制查詢所回傳的結果數量，或略過給定數量的查詢結果，你可以使用 `skip` 和 `take` 方法s:
...
有時你可能想要只有 else 為 true的時候使用子句來查詢。例如，如果傳入的請求中出現給定的輸入值，你可能就會想要使用 `where` 陳述句。你可以 `when` 方法來達成：
...
`when` 方法只有第一個參數為 `true` 時才執行給定的閉包。如果第一個參數為 `false`，該閉包將不會被執行。

你可以將另一個閉包作為第三個參數傳入 `when` 方法。這個閉包會在第一個參數結果為 `false` 時執行。為了說明如何使用此功能，我們使用它來設定預設的查詢排序：
