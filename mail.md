...
在 mailable 類別的 `build` 方法中，可以使用 `view` 方法來指定用來渲染郵件內容的模板。由於郵件一般是用 [Blade 模板](/docs/{{version}}/blade) 來渲染內容，可以藉由 Blade 模板引擎完整的功能和便利性來構建郵件的 HTML：
...
此方法會自動將工作加入隊列，以便在背景發送郵件訊息。當然，在使用此功能前需要[設定你的隊列](/docs/{{version}}/queues)。
