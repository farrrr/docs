...
通常情況下，您將會需要為您的路由產生 URL，您可以使用 `url` 助手來操作：
...
當然也可以在 Blade [模板引擎](/docs/master/templating)使用：
...
這裡有兩種方法從路由手動切換 404 錯誤。首先，您可以使用 `abort` 助手：

	abort(404);

 `abort` 助手只拋出 `Symfony\Component\HttpFoundation\Exception\HttpException` 帶有特定狀態代碼。
