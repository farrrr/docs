...
假設你的 HTTP 請求包含 「嵌套（nested）」 的參數，你可以指定他們在你的驗證規則中使用「.」語法：
...
在這個範例中，`:attribute` 透過驗證欄位的的實際名稱，預留的欄位會被取代。你還可以使用其他預設欄位的驗證訊息。例如：
    ...
    $messages = [
        ...
        'size'    => ':attribute 必須完全是 :size。',
        'between' => ':attribute 必須在 :min - :max 之間。',
...
驗證欄位是否是在指定日期之後。這個日期將會傳送到 PHP `strtotime` 函式：

    'start_date' => 'required|date|after:tomorrow'

而不是透過 `strtotime` 傳送日期字串來做評斷，你應該指定其他欄位來比較對照的日期：
...
驗證欄位必須符合給定的 _格式（ format ）_。此格式將會使用 PHP `date_parse_from_format` 函式做評斷。你應該使用 **任一** `date` 或 `date_format` 來驗證欄位，而不是兩者。
...
**注意：** 當使用 `regex`  pattern 時，你必須使用陣列，而不該用管線分隔規則，特別是如果正規表示式包含管線字元。
